/**
 * Scaffolding file used to store all the setups needed to run 
 * tests automatically generated by EvoSuite
 * Sat Aug 08 20:52:15 GMT 2026
 */

package zowe.client.sdk.zosfiles.dsn.methods;

import org.evosuite.runtime.annotation.EvoSuiteClassExclude;
import org.junit.BeforeClass;
import org.junit.Before;
import org.junit.After;
import org.junit.AfterClass;
import org.evosuite.runtime.sandbox.Sandbox;
import org.evosuite.runtime.sandbox.Sandbox.SandboxMode;

import static org.evosuite.shaded.org.mockito.Mockito.*;
@EvoSuiteClassExclude
public class DsnDelete_ESTest_scaffolding {

  @org.junit.Rule 
  public org.evosuite.runtime.vnet.NonFunctionalRequirementRule nfr = new org.evosuite.runtime.vnet.NonFunctionalRequirementRule();

  private static final java.util.Properties defaultProperties = (java.util.Properties) java.lang.System.getProperties().clone(); 

  private org.evosuite.runtime.thread.ThreadStopper threadStopper =  new org.evosuite.runtime.thread.ThreadStopper (org.evosuite.runtime.thread.KillSwitchHandler.getInstance(), 3000);


  @BeforeClass 
  public static void initEvoSuiteFramework() { 
    org.evosuite.runtime.RuntimeSettings.className = "zowe.client.sdk.zosfiles.dsn.methods.DsnDelete"; 
    org.evosuite.runtime.GuiSupport.initialize(); 
    org.evosuite.runtime.RuntimeSettings.maxNumberOfThreads = 100; 
    org.evosuite.runtime.RuntimeSettings.maxNumberOfIterationsPerLoop = 10000; 
    org.evosuite.runtime.RuntimeSettings.mockSystemIn = true; 
    org.evosuite.runtime.RuntimeSettings.sandboxMode = org.evosuite.runtime.sandbox.Sandbox.SandboxMode.RECOMMENDED; 
    org.evosuite.runtime.sandbox.Sandbox.initializeSecurityManagerForSUT(); 
    org.evosuite.runtime.classhandling.JDKClassResetter.init();
    setSystemProperties();
    initializeClasses();
    org.evosuite.runtime.Runtime.getInstance().resetRuntime(); 
    try { initMocksToAvoidTimeoutsInTheTests(); } catch(ClassNotFoundException e) {} 
  } 

  @AfterClass 
  public static void clearEvoSuiteFramework(){ 
    Sandbox.resetDefaultSecurityManager(); 
    java.lang.System.setProperties((java.util.Properties) defaultProperties.clone()); 
  } 

  @Before 
  public void initTestCase(){ 
    threadStopper.storeCurrentThreads();
    threadStopper.startRecordingTime();
    org.evosuite.runtime.jvm.ShutdownHookHandler.getInstance().initHandler(); 
    org.evosuite.runtime.sandbox.Sandbox.goingToExecuteSUTCode(); 
    setSystemProperties(); 
    org.evosuite.runtime.GuiSupport.setHeadless(); 
    org.evosuite.runtime.Runtime.getInstance().resetRuntime(); 
    org.evosuite.runtime.agent.InstrumentingAgent.activate(); 
  } 

  @After 
  public void doneWithTestCase(){ 
    threadStopper.killAndJoinClientThreads();
    org.evosuite.runtime.jvm.ShutdownHookHandler.getInstance().safeExecuteAddedHooks(); 
    org.evosuite.runtime.classhandling.JDKClassResetter.reset(); 
    resetClasses(); 
    org.evosuite.runtime.sandbox.Sandbox.doneWithExecutingSUTCode(); 
    org.evosuite.runtime.agent.InstrumentingAgent.deactivate(); 
    org.evosuite.runtime.GuiSupport.restoreHeadlessMode(); 
  } 

  public static void setSystemProperties() {
 
    java.lang.System.setProperties((java.util.Properties) defaultProperties.clone()); 
    java.lang.System.setProperty("file.encoding", "UTF-8"); 
    java.lang.System.setProperty("user.dir", "/home/suelenfelix/TCC/teste/analise-geracao-testes/projects/zowe_zowe-client-java-sdk"); 
  }

  private static void initializeClasses() {
    org.evosuite.runtime.classhandling.ClassStateSupport.initializeClasses(DsnDelete_ESTest_scaffolding.class.getClassLoader() ,
      "kong.unirest.core.Unirest",
      "com.google.common.collect.Hashing",
      "kong.unirest.core.ObjectMapper",
      "com.google.common.collect.ImmutableList$SubList",
      "com.google.common.collect.ListMultimap",
      "zowe.client.sdk.rest.DeleteJsonZosmfRequest",
      "com.google.common.collect.RegularImmutableList",
      "zowe.client.sdk.utility.ValidateUtils",
      "kong.unirest.core.Interceptor",
      "zowe.client.sdk.zosfiles.dsn.methods.DsnDelete$1",
      "zowe.client.sdk.core.AuthType",
      "com.google.common.collect.RegularImmutableMap$KeySet",
      "kong.unirest.core.WebSocketRequest",
      "kong.unirest.core.HttpRequest",
      "kong.unirest.core.RetryStrategy",
      "kong.unirest.core.NoopMetric",
      "zowe.client.sdk.zosfiles.dsn.methods.DsnDelete",
      "com.google.common.collect.Platform",
      "com.google.common.collect.BaseImmutableMultimap",
      "com.google.common.collect.RegularImmutableMap",
      "com.google.common.collect.ImmutableMultimap$Values",
      "kong.unirest.core.java.JavaClient",
      "zowe.client.sdk.rest.ZosmfHeaders",
      "com.google.common.collect.SortedMapDifference",
      "zowe.client.sdk.rest.GetTextZosmfRequest",
      "com.google.common.collect.AbstractMapEntry",
      "zowe.client.sdk.utility.EncodeUtils",
      "com.google.common.collect.ImmutableMap$IteratorBasedImmutableMap",
      "zowe.client.sdk.rest.PutStreamZosmfRequest",
      "com.google.common.collect.ImmutableListMultimap$Builder",
      "com.google.common.collect.ImmutableListMultimap",
      "com.google.common.collect.ImmutableMultimap$1",
      "kong.unirest.core.JsonPatchRequest",
      "com.google.common.collect.ImmutableMultimap$2",
      "kong.unirest.core.Cookie$SameSite",
      "com.google.common.collect.ImmutableAsList",
      "kong.unirest.core.Util",
      "com.google.common.collect.ImmutableMapEntrySet$RegularEntrySet",
      "zowe.client.sdk.rest.Response",
      "com.google.common.collect.ImmutableSet$CachingAsList",
      "com.google.common.collect.RegularImmutableAsList",
      "zowe.client.sdk.rest.GetStreamZosmfRequest",
      "com.google.common.collect.ImmutableMapEntrySet",
      "kong.unirest.core.HttpRequestWithBody",
      "com.google.common.collect.ImmutableMultiset",
      "com.google.common.collect.UnmodifiableListIterator",
      "com.google.common.collect.ImmutableMultimap$Keys",
      "zowe.client.sdk.zosfiles.dsn.input.DsnDeleteInputData",
      "com.google.common.collect.ImmutableMultimap",
      "zowe.client.sdk.rest.PutTextZosmfRequest",
      "zowe.client.sdk.rest.GetJsonZosmfRequest",
      "com.google.common.collect.ObjectArrays",
      "com.google.common.collect.Maps$BiMapConverter",
      "kong.unirest.core.UnirestParsingException",
      "kong.unirest.core.RawResponse",
      "kong.unirest.core.SseRequest",
      "com.google.common.collect.ImmutableList$1",
      "com.google.common.collect.MapDifference",
      "com.google.common.collect.ImmutableMap$MapViewOfValuesAsSingletonSets",
      "zowe.client.sdk.rest.exception.ZosmfRequestException",
      "com.google.common.collect.ImmutableMapEntry$NonTerminalImmutableMapEntry",
      "kong.unirest.core.UnirestConfigException",
      "com.google.common.base.Preconditions",
      "com.google.common.collect.UnmodifiableIterator",
      "kong.unirest.core.Cookies",
      "kong.unirest.core.UniMetric",
      "zowe.client.sdk.rest.type.ZosmfRequestType",
      "com.google.common.collect.ImmutableEntry",
      "zowe.client.sdk.zosfiles.dsn.types.DeleteType",
      "kong.unirest.core.DefaultInterceptor",
      "zowe.client.sdk.rest.ZosmfRequestFactory",
      "zowe.client.sdk.rest.PutJsonZosmfRequest",
      "com.google.common.collect.ImmutableEnumMap",
      "com.google.common.collect.ImmutableCollection",
      "kong.unirest.core.Config",
      "zowe.client.sdk.rest.ZosmfRequest$1",
      "com.google.common.collect.ImmutableMultisetGwtSerializationDependencies",
      "com.google.common.collect.ImmutableList$ReverseImmutableList",
      "com.google.common.collect.SingletonImmutableList",
      "com.google.common.base.Converter",
      "com.google.common.collect.Maps$8",
      "com.google.common.collect.BiMap",
      "zowe.client.sdk.core.ZosConnection",
      "com.google.common.collect.IndexedImmutableSet",
      "com.google.common.base.Function",
      "com.google.common.collect.ImmutableSet",
      "com.google.common.collect.ImmutableMap",
      "com.google.common.collect.ImmutableMapEntry",
      "com.google.common.collect.AbstractIndexedListIterator",
      "com.google.common.collect.CollectPreconditions",
      "kong.unirest.core.UnrecoverableException",
      "com.google.common.collect.ImmutableMultimap$EntryCollection",
      "kong.unirest.core.UnirestException",
      "zowe.client.sdk.rest.ZosmfRequest",
      "kong.unirest.core.Cookie",
      "com.google.common.collect.ImmutableMap$1",
      "com.google.common.collect.ImmutableMultimap$Builder",
      "kong.unirest.core.HttpResponse",
      "com.google.common.collect.Multiset",
      "kong.unirest.core.Client",
      "com.google.common.collect.AbstractMultimap",
      "com.google.common.collect.Multimap",
      "com.google.common.collect.EmptyImmutableListMultimap",
      "zowe.client.sdk.rest.ZosmfRequestFactory$1",
      "com.google.common.collect.ImmutableBiMap",
      "com.google.common.collect.ImmutableList",
      "kong.unirest.core.GetRequest",
      "com.google.common.collect.ImmutableBiMapFauxverideShim",
      "com.google.common.collect.RegularImmutableMap$BucketOverflowException",
      "kong.unirest.core.Headers",
      "kong.unirest.core.UnirestInstance",
      "zowe.client.sdk.rest.PostJsonZosmfRequest",
      "com.google.common.collect.Maps",
      "com.google.common.collect.ImmutableMap$Builder",
      "kong.unirest.core.HttpRequestSummary",
      "com.google.common.collect.Maps$EntryTransformer",
      "com.google.common.collect.RegularImmutableMap$Values",
      "kong.unirest.core.CompoundInterceptor",
      "kong.unirest.core.Body"
    );
  } 
  private static void initMocksToAvoidTimeoutsInTheTests() throws ClassNotFoundException { 
    mock(Class.forName("zowe.client.sdk.core.ZosConnection", false, DsnDelete_ESTest_scaffolding.class.getClassLoader()));
  }

  private static void resetClasses() {
    org.evosuite.runtime.classhandling.ClassResetter.getInstance().setClassLoader(DsnDelete_ESTest_scaffolding.class.getClassLoader()); 

    org.evosuite.runtime.classhandling.ClassStateSupport.resetClasses(
      "zowe.client.sdk.zosfiles.dsn.methods.DsnDelete",
      "zowe.client.sdk.rest.type.ZosmfRequestType",
      "zowe.client.sdk.rest.ZosmfRequestFactory",
      "zowe.client.sdk.rest.ZosmfRequestFactory$1",
      "zowe.client.sdk.zosfiles.dsn.types.DeleteType",
      "zowe.client.sdk.zosfiles.dsn.methods.DsnDelete$1",
      "com.google.common.collect.ImmutableMultimap$Builder",
      "com.google.common.collect.ImmutableListMultimap$Builder",
      "com.google.common.collect.Platform",
      "com.google.common.collect.Maps",
      "com.google.common.collect.CollectPreconditions",
      "com.google.common.collect.AbstractMultimap",
      "com.google.common.collect.BaseImmutableMultimap",
      "com.google.common.collect.ImmutableMultimap",
      "com.google.common.collect.ImmutableListMultimap",
      "com.google.common.collect.ImmutableMap$Builder",
      "com.google.common.collect.ImmutableCollection",
      "com.google.common.collect.ImmutableList",
      "com.google.common.collect.ObjectArrays",
      "com.google.common.collect.SingletonImmutableList",
      "com.google.common.base.Preconditions",
      "com.google.common.collect.ImmutableMap",
      "com.google.common.collect.AbstractMapEntry",
      "com.google.common.collect.ImmutableEntry",
      "com.google.common.collect.ImmutableMapEntry",
      "com.google.common.collect.RegularImmutableList",
      "com.google.common.collect.RegularImmutableMap",
      "com.google.common.collect.Hashing",
      "com.google.common.collect.ImmutableMapEntry$NonTerminalImmutableMapEntry",
      "zowe.client.sdk.rest.ZosmfHeaders",
      "zowe.client.sdk.rest.ZosmfRequest",
      "zowe.client.sdk.rest.PostJsonZosmfRequest",
      "zowe.client.sdk.utility.ValidateUtils",
      "zowe.client.sdk.core.ZosConnection",
      "zowe.client.sdk.rest.PutStreamZosmfRequest",
      "zowe.client.sdk.rest.DeleteJsonZosmfRequest",
      "zowe.client.sdk.rest.GetStreamZosmfRequest",
      "zowe.client.sdk.zosfiles.dsn.input.DsnDeleteInputData",
      "zowe.client.sdk.utility.EncodeUtils",
      "zowe.client.sdk.rest.GetJsonZosmfRequest",
      "zowe.client.sdk.rest.PutJsonZosmfRequest",
      "zowe.client.sdk.rest.PutTextZosmfRequest",
      "kong.unirest.core.UnirestInstance",
      "kong.unirest.core.Config",
      "kong.unirest.core.NoopMetric",
      "kong.unirest.core.CompoundInterceptor",
      "kong.unirest.core.DefaultInterceptor",
      "kong.unirest.core.Headers",
      "kong.unirest.core.Unirest",
      "zowe.client.sdk.rest.ZosmfRequest$1",
      "zowe.client.sdk.rest.GetTextZosmfRequest",
      "kong.unirest.core.Util",
      "kong.unirest.core.UnirestException",
      "kong.unirest.core.UnirestConfigException",
      "kong.unirest.core.Cookie"
    );
  }
}
