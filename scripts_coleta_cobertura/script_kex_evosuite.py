...
def inject_assets_kex_evosuite(pom_path):
...
def isolate_native_tests(project_path):
    """
    Move:
      src/test/java -> src/test/java_backup_native
    para impedir execução/compilação dos testes nativos.
    """
...
def process_project(project_path):
    ...
    project_csv = project_path / "coverage_kex_evosuite.csv"
    log_file = project_path / "coverage_kex_evosuite.log"

    print("=" * 80)
    print(f"🚀 Processando apenas Kex + EvoSuite: {project_name}")
    print("=" * 80)
    ...
    try:
        ...
        inject_assets_kex_evosuite(pom)
        ...
        if metrics:
            ...
            print(f"✅ Cobertura Kex + EvoSuite: {coverage}%")
