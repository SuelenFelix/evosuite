...
CSV_FILE = "coverage_native_kex.csv"
...
def move_kex_to_test_source(project_path):
    ...
    Move temporariamente:
      <project>/kex-tests
    para:
      <project>/src/test/java/kex-tests
    """
    ...
    if not root_kex.exists():
        print("ℹ️ Pasta kex-tests não encontrada na raiz do projeto.")
        return None, None
...
def main():
    ...
    try:
        temp_kex_path, original_kex_path = move_kex_to_test_source(path)
        ...
        if metrics:
            print(f"✅ Cobertura Nativos + Kex: {metrics['instruction_coverage']}%")
            save_result(
                CSV_FILE,
                project_name,
                metrics["instruction_coverage"],
                mode="nativo+kex"
            )
        else:
